...
class MongoClient:
    ...
    async def get_many(self, collection: str, query: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Retrieve multiple documents."""
        logger.debug(f"Fetching multiple documents from {collection} with query: {query}")
    ...
    async def update_document(self, collection: str, query: Dict[str, Any], update: Dict[str, Any]) -> UpdateResult:
        """Update a document."""
        logger.debug(f"Updating document in {collection} with query: {query} and update: {update}")
        ...
        result: UpdateResult = await self.get_collection(collection).update_one(query, update_query)
        logger.debug(f"Matched {result.matched_count}, modified {result.modified_count} documents.")
        return result


    async def delete_document(self, collection: str, query: Dict[str, Any]) -> DeleteResult:
        """Delete a document."""
