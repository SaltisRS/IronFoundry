...
async def set_invite_permissions(
    channel: discord.VoiceChannel, invited: discord.Member
):
...
class PromptView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Use Default", style=discord.ButtonStyle.green, custom_id="default_vc_btn"
    )
    async def use_default(
        ...
        try:
            category = interaction.guild.get_channel(vc_cat)
            channel = await category.create_voice_channel(
                name=interaction.user.display_name, position=1
            )

            add_active_channel(channel, interaction.user.id)

            await interaction.response.send_message(
                f"Created channel: **{channel.name}**", ephemeral=True, delete_after=10
            )
    ...
    @discord.ui.button(
        label="Configure", style=discord.ButtonStyle.blurple, custom_id="config_vc_btn"
    )
    async def configure_vc(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        if has_active_channel(interaction.user.id):
            await interaction.response.send_message(
...
async def vc_inv(interaction: discord.Interaction, user: discord.Member):
    if not has_active_channel(interaction.user.id):
        await interaction.response.send_message(
            "You need to create a custom Voice Channel to use this command.",
            ephemeral=True,
            delete_after=5,
        )
        return
    ...
    if not is_users_channel(vc.channel, interaction.user):
        await interaction.response.send_message(
            "You must be in your own custom Voice Channel to use this command.",
            ephemeral=True,
            delete_after=5,
        )
        return
    ...
    try:
        await user.send(
            f"You have been invited to join: {vc.channel.name}\n{vc.channel.jump_url}"
        )
    except Exception as e:
        logger.error(f"Couldnt Message User\n{e}")

    await interaction.response.send_message(
        f"Invited: {user.display_name}", ephemeral=True, delete_after=5
    )
