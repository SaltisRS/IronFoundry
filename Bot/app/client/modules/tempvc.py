...
class PromptModal(discord.ui.Modal, title="VC Settings"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        channel = await category.create_voice_channel(name=self.name.value if self.name.value else interaction.user.name, options={"user_limit": int(self.limit.value) if self.limit.value else None})
